...
**버전**: v0.4
**발행일**: 2026년 5월 1일
**문의**: sajuastrology@gmail.com

---

### 저작권 및 사용 조건

본 백서의 저작권은 림팩토리(Rimfactory)에게 있다. 학술·언론·비상업적 연구 목적의 인용은 저자와 림팩토리의 출처를 명시할 경우 허용된다. 본 문서에 기술된 시스템·알고리즘·방법·데이터셋·제품 개념의 상업적 재사용, 복제, 번역, 파생물 배포, 재구현은 림팩토리의 사전 서면 동의를 요한다.

본 문서는 기술적·디지털인문학적 연구 방향을 기록하기 위한 것이다. 본 문서에 기술된 일부 시스템 설계, 데이터 구조, 평가 방법, 제품 구현은 향후 지식재산권 검토·보호의 대상이 될 수 있다.

ⓒ 2026 Rimfactory. All rights reserved.

---

### 범위 및 면책 (Scope and Disclaimer)

본 백서는 결정론적 예측, 전문적 자문, 또는 인생 결과에 대한 과학적 증명을 주장하지 않는다. 본 시스템은 문화적 해석, 자기 성찰, 개인 통찰 기술로 자리매김한다. 본 시스템은 의료, 법률, 재무, 심리적, 또는 그 밖의 전문적 자문을 제공하지 않는다.

본 문서에서 사용되는 장기 연구 언어 — 신호 품질, 평가, 검증, 사용자 경향 신호 등의 용어 — 는 투명한 평가 방법을 통해 시험되어야 할 *가설과 공학적 목표*로 이해되어야 하며, 보장된 예측 정확도에 대한 주장으로 읽혀서는 안 된다.
...
본 연구가 제안하는 핵심은 다음 다섯 가지다. 첫째, 명리 정전 코퍼스를 고차원 임베딩 공간에 사상하여 학파 간 해석의 거리를 정량적으로 측정한다. 둘째, 사주의 구조를 그래프 형태로 명시하여 결정론적 분석과 LLM 추론을 결합한다. 셋째, 통합 검색 증강 생성(Integrated RAG) 아키텍처로 명리 풀이를 재현 가능하고 추적 가능한 형태로 생성한다. 넷째, 약한 검증과 중간 검증의 두 수준에서 평가 프로토콜을 정의하고 평가 방법론과 선별된 요약 결과를 공개한다. 다섯째, *분절적 점수화 접근*(오행 점수, 십신 가중 평균, 단일 좌표 환원)을 명시적으로 거부하고, 잠재 공간 기반의 *분절 우회* 전략을 채택한다.

본 백서는 이 작업을 RimSaju v1·v2·v3의 3단계 빌드로 정의한다. v1은 이미 운영 중인 프로덕션 RAG 기반 명리 엔진이며, v2는 2년간의 빌드(2026.Q3 ~ 2028.Q2)로 본 백서가 약속한 6개 층 모두를 완성한다. v3는 추가 2년간의 빌드(2028.Q3 ~ 2030.Q2)로 깊이의 확장과 통계적 검증을 진입한다. 본 백서는 v1·v2·v3 전 단계에 걸친 시스템 설계와 지식재산권 보호 전략을 함께 제시한다.
...
- 제13장. RimSaju v1 — 프로덕션 RAG 기반 명리 엔진
...
- 평가 방법론과 선별된 요약 결과를 — 적중 여부와 무관하게 — 공개한다.
- 학설이 반증되면 그 사실을 기록한다.
...
본 연구는 RimSaju를 v1, v2, v3의 3단계로 구성한다. v1은 이미 운영 중인 프로덕션 RAG 기반 명리 엔진이며(제13장), v2는 2년간의 빌드(2026.Q3 ~ 2028.Q2)로 본 백서가 약속한 6개 층 모두를 *최소 가용 형태로* 완성한다. v3는 v2의 토대 위에서 추가 2년간의 빌드(2028.Q3 ~ 2030.Q2)로 *깊이의 확장*을 수행한다. 본 장은 4년 전체 로드맵과 단계별 산출물을 명시한다.

3단계의 위상은 다음과 같이 정의된다.

| 단계 | 기간 | 핵심 정체성 | 종료 시점 시스템 상태 |
|------|------|------------|--------------------|
| v1 | ~2026.Q2 (운영 중) | 프로덕션 RAG 기반 명리 엔진 | 562 패시지 단순 검색 RAG, 5단계 폴백, 99.9% 가용성 |
...
### 14-7. 지식재산권 및 데이터 거버넌스 고려

본 백서가 기술하는 시스템은 소프트웨어 아키텍처, 큐레이션된 텍스트 처리, 검색 설계, 평가 방법, 제품 구현을 결합한다. 림팩토리는 시스템이 프로덕션 환경에서 성숙해짐에 따라 일부 구성 요소에 대해 향후 지식재산권 보호를 검토할 수 있다.

본 공개 버전은 독점 소스 코드, 비공개 데이터셋, 보안 민감 구현 세부 사항, 또는 청구항 수준의 기술 명세를 공개하지 않는다. 본 백서의 목적은 인용·기술 토론·디지털인문학적 검토에 적합한 수준에서 연구 방향과 시스템 아키텍처를 문서화하는 데 있다.

데이터셋 거버넌스는 시스템 아키텍처와 별개로 다룬다. 고전 텍스트, 현대 주석서, 사용자 동의 기반 평가 데이터, 제품 텔레메트리는 각각 상이한 법적·윤리적 제약을 받는다. 림팩토리는 이러한 자료들을 적절한 저작권 클리어런스, 동의 관리, 비식별화, 라이선스 관행을 통해 다루고자 한다.

지도 원칙은 공개적 연구 공개와 독점 구현 세부 사항 및 사용자 데이터의 책임 있는 보호 사이의 균형을 취하는 것이다.
...
**v1 (운영 중)** — 프로덕션 RAG 기반 명리 엔진. 562 패시지 정전 임베딩 데이터베이스. 5단계 LLM 폴백. 한·영·일 3언어 풀이.
...
본 백서를 끝까지 읽으신 독자께서 위의 어느 영역에 속하신다면 림팩토리에 연락해 주시기를 청한다. sajuastrology@gmail.com.
...
본 부록은 본 백서의 공개 내용과 공개 범위 외의 자료 사이의 경계를 명확히 한다.

본 공개판은 RimSaju의 개념적 아키텍처, 역사적 프레이밍, 평가 방향, 제품 로드맵을 기술한다. 본 공개판은 독점 소스 코드, 비공개 평가 데이터, 비공개 데이터셋, 보안 민감 구현 세부 사항, 또는 향후 지식재산권 출원을 위한 청구항 수준의 명세는 공개하지 않는다.

향후 검토가 필요할 수 있는 영역은 다음과 같다: 시스템 아키텍처, 검색 및 랭킹 방법, 사주 표현 구조, 평가 워크플로, 용어 사전, 데이터셋 큐레이션 방법, 제품 구현 세부 사항. 보호의 형태가 있다면, 그것은 추가적인 기술 개발, 법적 검토, 프로덕션 검증을 거친 이후에만 결정될 것이다.

본 백서의 목적은 인용, 투명성, 기술 토론, 디지털인문학적 자리매김을 지원하는 데 있다. 본 백서는 림팩토리의 독점 구현의 완전한 공개로 해석되어서는 안 된다.
...
**Inquiries**: sajuastrology@gmail.com

---

### Copyright and Terms of Use

Copyright (C) 2026 Rimfactory. All rights reserved. Citation for academic, journalistic, and non-commercial research purposes is permitted with clear attribution to the author and Rimfactory. Commercial reuse, reproduction, translation, derivative distribution, or re-implementation of the systems, algorithms, methods, datasets, or product concepts described in this document requires prior written permission from Rimfactory.

This document is intended to document a technical and digital-humanities research direction. Certain system designs, data structures, evaluation methods, and product implementations described here may be subject to future intellectual-property review or protection.
...
This whitepaper does not claim deterministic prediction, professional advice, or scientific proof of life outcomes. The system is positioned as a cultural interpretation, self-reflection, and personal insight technology. It does not provide medical, legal, financial, psychological, or other professional advice.

The long-term research language used in this document — including terms such as signal quality, evaluation, validation, and user-tendency signal — should be understood as *hypotheses and engineering goals* to be tested through transparent evaluation methods. They should not be read as claims of guaranteed predictive accuracy.
...
- Chapter 13. RimSaju v1 — The Production RAG-Based Saju Engine
...
- Appendix E. Public Disclosure and IP Review Scope
...
- To publish evaluation methodology and selected summary results — regardless of whether the predictions are confirmed.
...
### 14-7. Intellectual-Property and Data Governance Considerations

The system this whitepaper describes combines software architecture, curated text processing, retrieval design, evaluation methods, and product implementation. Rimfactory may review selected components for future intellectual-property protection as the system matures in production.

This public version does not disclose proprietary source code, private datasets, security-sensitive implementation details, or claim-level technical specifications. The purpose of this whitepaper is to document the research direction and system architecture at a level suitable for citation, technical discussion, and digital-humanities review.

Dataset governance is treated separately from system architecture. Classical texts, modern commentaries, user-consented evaluation data, and product telemetry are subject to different legal and ethical constraints. Rimfactory intends to handle such materials through appropriate copyright clearance, consent management, anonymization, and licensing practices.

The guiding principle is to balance public research disclosure with responsible protection of proprietary implementation details and user data.
...
**v1 (in operation)** — A production RAG-based *mingli* engine. A canonical-text embedding database of 562 passages. A five-stage LLM fallback. Readings in three languages: Korean, English, Japanese.
...
If a reader who has read this whitepaper to the end belongs to any of the areas above, we ask you to write to Rimfactory. sajuastrology@gmail.com.
...
This appendix clarifies the boundary between the public content of this whitepaper and materials that remain outside the scope of public disclosure.

This public release describes the conceptual architecture, historical framing, evaluation direction, and product roadmap of RimSaju. It does not disclose proprietary source code, private evaluation data, non-public datasets, security-sensitive implementation details, or claim-level specifications for any future intellectual-property filings.

Areas that may require future review include: system architecture, retrieval and ranking methods, chart-representation structures, evaluation workflows, terminology dictionaries, dataset curation methods, and product implementation details. The form of protection, if any, will be determined only after further technical development, legal review, and production validation.

The purpose of this whitepaper is to support citation, transparency, technical discussion, and digital-humanities positioning. It should not be interpreted as a complete disclosure of Rimfactory's proprietary implementation.
...
The copyright of this whitepaper belongs to Rimfactory. Citation for academic, journalistic, and non-commercial research purposes is permitted with clear attribution to the author and Rimfactory. Commercial reuse, reproduction, translation, derivative distribution, or re-implementation of the systems, algorithms, methods, datasets, or product concepts described in this document requires prior written permission from Rimfactory.
...
**バージョン**: v0.4
**発行日**: 2026 年 5 月 1 日
**お問い合わせ**: sajuastrology@gmail.com

---

### 著作権および利用条件

本ホワイトペーパーの著作権はリムファクトリー (Rimfactory) に帰属する。学術・報道・非商業的研究目的での引用は、著者およびリムファクトリーの出典を明示する場合に限り許可する。本書に記述されたシステム・アルゴリズム・方法・データセット・製品コンセプトの商業的再使用、複製、翻訳、派生物の配布、再実装は、リムファクトリーの事前の書面による許可を要する。

本書は、技術的および人文学的なデジタル研究の方向性を記録することを目的とする。本書に記述された一部のシステム設計、データ構造、評価方法、製品実装は、将来的な知的財産権の検討・保護の対象となる可能性がある。

ⓒ 2026 Rimfactory. All rights reserved.

---

### 範囲および免責 (Scope and Disclaimer)

本ホワイトペーパーは、決定論的予測、専門的助言、または人生の結果に関する科学的証明を主張するものではない。本システムは、文化的解釈、自己省察、個人洞察の技術として位置づけられる。本システムは、医療、法律、財務、心理的、その他の専門的助言を提供しない。

本書で用いられる長期的な研究用語 — シグナル品質、評価、検証、ユーザー傾向シグナル等の用語 — は、透明な評価方法を通じて検証されるべき *仮説および工学的目標* として理解されるべきであり、保証された予測精度に関する主張として読まれてはならない。
...
本研究が提案する核心は次の五点である。第一に、命理正典コーパスを高次元埋め込み空間へ写像し、学派間解釈の距離を定量的に測定する。第二に、四柱推命の構造をグラフ形式で明示することにより、決定論的分析と LLM 推論を結合する。第三に、統合検索拡張生成 (Integrated RAG) アーキテクチャによって命理解釈を再現可能かつ追跡可能な形で生成する。第四に、弱い検証と中間検証の二水準で評価プロトコルを定義し、評価方法論および選別された要約結果を公開する。第五に、*分節的スコア化アプローチ* (五行スコア、十神加重平均、単一座標への還元) を明示的に拒否し、潜在空間に基づく *分節の迂回* 戦略を採用する。

本ホワイトペーパーは、この作業を RimSaju v1·v2·v3 の三段階ビルドとして定義する。v1 はすでに運用中のプロダクション RAG ベース命理エンジンであり、v2 は二年間のビルド (2026.Q3 ~ 2028.Q2) によって本書が約束する六つの層すべてを完成させる。v3 はさらに二年間のビルド (2028.Q3 ~ 2030.Q2) によって、深さの拡張と統計的検証への移行を行う。本書は v1·v2·v3 のすべての段階にまたがるシステム設計と、知的財産権保護戦略を併せて提示する。
...
- 第十三章. RimSaju v1 — プロダクション RAG ベース命理エンジン
...
- 評価方法論および選別された要約結果を — 的中するか否かに関わらず — 公開する。
- 学説が反証されればその事実を記録する。
...
本研究は RimSaju を v1、v2、v3 の三段階に構成する。v1 はすでに運用中のプロダクション RAG ベース命理エンジンであり (第十三章)、v2 は二年間のビルド (2026.Q3 ~ 2028.Q2) によって本書が約束した 6 つの層すべてを *最小可用形態で* 完成させる。v3 は v2 の基盤の上で、追加の二年間のビルド (2028.Q3 ~ 2030.Q2) によって *深さの拡張* を遂行する。本章は 4 年全体のロードマップと段階別の成果物を明示する。

三段階の位相は次のように定義される。

| 段階 | 期間 | 核心的アイデンティティ | 終了時点のシステム状態 |
|------|------|------------|--------------------|
| v1 | ~2026.Q2 (運用中) | プロダクション RAG ベース命理エンジン | 562 パッセージの単純検索 RAG、5 段階フォールバック、99.9 % 可用性 |
...
### 14-7. 知的財産権およびデータガバナンスの考察

本書が記述するシステムは、ソフトウェアアーキテクチャ、キュレーションされたテキスト処理、検索設計、評価方法、製品実装を結合したものである。リムファクトリーは、システムがプロダクション環境において成熟するにつれて、一部の構成要素について将来的な知的財産権保護を検討する可能性がある。

本公開版は、独占的なソースコード、非公開のデータセット、セキュリティ機微な実装の詳細、または請求項レベルの技術仕様を開示しない。本書の目的は、引用、技術的議論、デジタル人文学的検討に適した水準で、研究方向およびシステムアーキテクチャを文書化することにある。

データセット・ガバナンスは、システムアーキテクチャとは別に扱う。古典テキスト、現代の注釈書、ユーザーの同意に基づく評価データ、製品テレメトリは、それぞれ異なる法的・倫理的制約を受ける。リムファクトリーは、これらの資料を、適切な著作権クリアランス、同意管理、匿名化、ライセンス慣行を通じて扱うことを意図する。

指導原則は、公開的な研究開示と、独占的実装の詳細およびユーザーデータの責任ある保護との間でバランスを取ることである。
...
**v1 (運用中)** — プロダクション RAG ベース命理エンジン。562 パッセージの正典埋め込みデータベース。5 段階の LLM フォールバック。韓・英・日 3 言語の解釈。
...
本書を最後までお読みになった読者の方が、上記いずれかの領域に属する方であれば、リムファクトリーへご連絡いただきたい。sajuastrology@gmail.com。
...
本付録は、本ホワイトペーパーの公開内容と、公開範囲外に位置する資料との境界を明確にする。

本公開版は RimSaju の概念的アーキテクチャ、歴史的位置づけ、評価の方向性、製品ロードマップを記述する。本公開版は、独占的ソースコード、非公開の評価データ、非公開のデータセット、セキュリティ機微な実装の詳細、または将来の知的財産権出願に向けた請求項レベルの仕様は開示しない。

将来的な検討の対象となる可能性のある領域は次の通りである: システムアーキテクチャ、検索およびランキング手法、四柱推命の表現構造、評価ワークフロー、用語辞典、データセット・キュレーション手法、製品実装の詳細。保護の形式がある場合、それは追加的な技術開発、法的検討、プロダクション検証を経た後にのみ決定される。

本ホワイトペーパーの目的は、引用、透明性、技術的議論、デジタル人文学的位置づけを支援することにある。本書はリムファクトリーの独占的実装の完全な開示として解釈されてはならない。
