...
// ÏΩîÌçº???úÎî© API (?êÌ??? admin??
// POST /api/rag/seed?key=SERVICE_KEY ??corpus-data.json???ÑÎ≤†?©Ìïò??Supabase???Ä??// POST /api/rag/seed?key=SERVICE_KEY&batch=3 ???πÏ†ï Î∞∞ÏπòÎ∂Ä???¥Ïñ¥?òÍ∏∞
...
export const maxDuration = 300; // 5Î∂?(Vercel Serverless)
...
export async function POST(request: NextRequest) {
  try {
    // ?∏Ï¶ù: service keyÎ•?ÏøºÎ¶¨ ?åÎùºÎØ∏ÌÑ∞Î°?    const { searchParams } = new URL(request.url);
    ...
    for (let i = startBatch * BATCH_SIZE; i < chunks.length; i += BATCH_SIZE) {
      ...
      // Rate limiting (OpenAI Î∂ÑÎãπ ?úÌïú Î∞©Ï?)
      await new Promise(r => setTimeout(r, 500));
    }

    // ÏµúÏ¢Ö Ïπ¥Ïö¥??    const { count } = await supabase
      .from('sj_corpus_chunks')
...
export async function GET(request: NextRequest) {
  try {
    ...
    // ?åÏä§Î≥?Ïπ¥Ïö¥??    const { data: sourceCounts } = await supabase
      .from('sj_corpus_chunks')
