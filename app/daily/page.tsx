...
const txt = {
  pageTitle: { en: "Today's Fortune", ko: "오늘의 운세", ja: "今日の運勢" },
  todayEnergy: { en: "Today's Energy", ko: "오늘의 기운", ja: "今日のエネルギー" },
  dominantElement: { en: "Dominant Element", ko: "지배 원소", ja: "支配元素" },
  yourScore: { en: "Your Energy Score", ko: "나의 에너지 점수", ja: "あなたのエネルギー" },
  fortune: { en: "Your Daily Fortune", ko: "오늘의 운세 메시지", ja: "今日の運勢メッセージ" },
  luckyItems: { en: "Lucky Items", ko: "행운 아이템", ja: "ラッキーアイテム" },
  color: { en: "Color", ko: "색상", ja: "カラー" },
  direction: { en: "Direction", ko: "방향", ja: "方角" },
  activity: { en: "Activity", ko: "활동", ja: "活動" },
  weekAhead: { en: "Week Ahead", ko: "이번 주 에너지", ja: "今週のエネルギー" },
  getReading: { en: "Get Your Free Reading", ko: "무료 사주 보기", ja: "無料で鑑定する" },
  personalCta: {
  ...
  excellent: { en: "Excellent day ahead!", ko: "최고의 하루!", ja: "素晴らしい一日!" },
  balanced: { en: "A balanced, steady day.", ko: "균형 잡힌 하루.", ja: "バランスの取れた一日。" },
  beGentle: { en: "Take it easy today.", ko: "오늘은 무리하지 마세요.", ja: "今日はゆっくり過ごしましょう。" },
  upgrade: { en: "Unlock Full Destiny Reading", ko: "전체 운명 분석 잠금 해제", ja: "フルデスティニーリーディングを解放" },
  upgradeDesc: {
...
const ELEMENT_NAMES: Record<string, Record<string, string>> = {
  wood: { en: "Wood", ko: "목(木)", ja: "木" },
  fire: { en: "Fire", ko: "화(火)", ja: "火" },
  earth: { en: "Earth", ko: "토(土)", ja: "土" },
  metal: { en: "Metal", ko: "금(金)", ja: "金" },
  water: { en: "Water", ko: "수(水)", ja: "水" },
};
                ...
                <div className="bg-card/50 backdrop-blur border border-primary/20 rounded-2xl p-6">
                  ...
                  <div className="border-t border-border pt-4">
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      <div className="flex flex-col items-center gap-1.5 p-2 rounded-lg bg-muted/20">
                        <div className="w-5 h-5 rounded-full border border-border" style={{ backgroundColor: fortune.luckyColorHex }} />
                        <span className="text-xs text-foreground">{fortune.luckyColor}</span>
                        <span className="text-[10px] text-muted-foreground">{tx(txt.color, locale)}</span>
                      </div>
                      <div className="flex flex-col items-center gap-1.5 p-2 rounded-lg bg-muted/20">
                        <Compass className="w-5 h-5 text-muted-foreground" />
                        <span className="text-xs text-foreground">{fortune.luckyDirection}</span>
                        <span className="text-[10px] text-muted-foreground">{tx(txt.direction, locale)}</span>
                      </div>
                      <div className="flex flex-col items-center gap-1.5 p-2 rounded-lg bg-muted/20">
                        <Zap className="w-5 h-5 text-muted-foreground" />
                        <span className="text-xs text-foreground text-center leading-tight">{fortune.luckyActivity}</span>
                        <span className="text-[10px] text-muted-foreground">{tx(txt.activity, locale)}</span>
                      </div>
                    </div>
                  </div>
                </div>
